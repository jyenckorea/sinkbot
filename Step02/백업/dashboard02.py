# 파일명: dashboard.py (v4.1 - 모델 리로드 버튼 추가)
...
st.title("🛰️ SinkBot AI 관제 대시보드 (v4.1)")

# --- ⭐️ 사이드바에 모델 관리 기능 추가 ⭐️ ---
with st.sidebar:
    st.header("⚙️ 모델 관리")
    if st.button("새 AI 모델 불러오기 (Reload)"):
        # st.cache_resource에 저장된 모든 캐시를 지웁니다.
...
try:
    ...
    st.header("🚨 시스템 상태")
    if model is None:
        st.warning(f"'{model_file}' AI 모델 파일을 찾을 수 없습니다. `trainer.py`를 먼저 실행하여 모델을 생성해주세요.", icon="⚠️")
    elif len(df) < 2:
    ...
    if df_processed is not None:
        ...
        with col1:
            ...
            folium.Marker(
                ...
                tooltip="현재 측정 위치", icon=folium.Icon(color='red', icon='tower', prefix='fa')
            ).add_to(m)
