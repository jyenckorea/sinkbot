# 파일명: dashboard.py (v1.9 - 비활성화 방식으로 잔상 문제 최종 해결)
...
st.title("🛰️ SinkBot AI 관제 대시보드 (v1.9)")

# --- 사이드바 ---
with st.sidebar:
    st.header("⚙️ 제어판")
    auto_refresh = st.toggle("자동 새로고침 활성화 ⏱️", value=True)
    if auto_refresh: st.caption("현재 상태: 5초마다 자동 새로고침 중...")
    else: st.caption("현재 상태: 수동 새로고침 모드.")
