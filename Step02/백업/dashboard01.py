# 파일명: dashboard.py (v3.2 - Folium 지도 적용)
...
st.title("🛰️ SinkBot 분석 대시보드 (v3.2)")

db_file = 'sinkbot_data.db'

def process_data(df):
    """ 데이터프레임을 받아 기준점 대비 변위를 계산합니다. """
    if len(df) < 1:
...
try:
    ...
    if df_processed is not None:
        ...
        with col1:
            ...
            # Folium 지도를 생성 (시작 위치를 현재 위치로 설정)
            m = folium.Map(location=[lat, lon], zoom_start=16)

            # 마커 추가 (클릭 시 팝업 정보 표시)
            folium.Marker(
                [lat, lon], 
                popup=f"<b>SinkBot</b><br>위도: {lat:.5f}<br>경도: {lon:.5f}",
                tooltip="현재 측정 위치",
                # ⭐️ 아이콘 설정 추가
