# 파일명: dashboard.py (v4.0 - Session State 최종 안정화)
...
st.title("🛰️ SinkBot AI 관제 대시보드 (v4.0)")

# --- ⭐️ 1. Session State 초기화 ⭐️ ---
# 앱이 처음 실행될 때 한 번만 실행됩니다.
...
with st.sidebar:
    ...
    # ⭐️ 2. 새로고침 간격 조절 기능 추가
    refresh_interval = st.select_slider(
        "새로고침 간격 (초)",
        options=[5, 10, 15, 30, 60],
        value=10 # 기본값을 10초로 늘림
    )
    if auto_refresh:
...
try:
    ...
    st.dataframe(df.tail(20).iloc[::-1]) # 20개로 늘림
