# 파일명: dashboard.py (v2.2 - Javascript를 이용한 최종 해결)
...
st.title("🛰️ SinkBot AI 관제 대시보드 (v2.2)")

# --- 사이드바 ---
with st.sidebar:
    st.header("⚙️ 제어판")
    auto_refresh = st.toggle("자동 새로고침 활성화 ⏱️", value=True)
    refresh_interval = 5 # 새로고침 간격(초)
    if auto_refresh:
        st.caption(f"현재 상태: {refresh_interval}초마다 자동 새로고침 중...")
    else:
    ...
    if df_processed is not None:
        ...
        with col2:
            ...
            if 'delta_z' in df_processed.columns and len(df_processed) > 1:
                ...
                with graph_col1:
                    time_range = st.selectbox("시간 범위 선택", options=['전체', '최근 24시간', '최근 6시간', '최근 1시간'])
                with graph_col2:
                    agg_level = st.selectbox("데이터 집계 방식", options=['원본 데이터', '시간별 평균', '일별 평균'])
                ...
                with axis_col1:
                    ...
                    user_start_date = st.date_input("시작 날짜:", value=min_date, min_value=min_date, max_value=max_date, disabled=not manual_axis_control)
                    user_end_date = st.date_input("종료 날짜:", value=max_date, min_value=min_date, max_value=max_date, disabled=not manual_axis_control)
                with axis_col2:
                    st.markdown("**Y축 범위 (침하량)**")
                    min_val, max_val = float(df_to_plot['delta_z'].min()), float(df_to_plot['delta_z'].max())
                    user_min = st.number_input("최소값:", value=min_val, format="%.4f", disabled=not manual_axis_control)
                    user_max = st.number_input("최대값:", value=max_val, format="%.4f", disabled=not manual_axis_control)

                x_axis_range, y_axis_range = None, None
                if manual_axis_control:
...
# st.rerun() 대신 Javascript를 이용한 브라우저 새로고침
if auto_refresh:
    # Javascript 코드: 지정된 시간(밀리초) 후에 페이지를 새로고침합니다.
    js_code = f"""
        <script>
            setTimeout(function() {{
    ...
    """
    # html 컴포넌트를 이용해 Javascript를 페이지에 주입합니다.
    html(js_code)
# --- ⭐️ 변경 끝 ⭐️ ---